...
export const componentWithPlaceholder = () => {
    let componentWithPlaceholder = vscode.commands.registerCommand(
        "sitecore-xm-cloud-components.insertComponentWithPlaceholder",
        () => {
    ...
    const ${componentName} = ({ fields, params, rendering }: ${propsTypeName}) => {
        return (
            <section className={params.styles}>
                ...
                <Placeholder name="$2" rendering={rendering} />
                $3
            </section>
        );
    };
    ...
    export default withDatasourceCheck()<${propsTypeName}>(${componentName});`;
      ...
      return componentWithPlaceholder
}